...
class QuizService {
  ...
  Future<List<Quiz>> getCommunityQuizzes(int communityId) async {
    ...
    final response = await http.get(
      Uri.parse('https://375a-217-55-17-184.ngrok-free.app/api/v1/quizzes/community/$communityId'),
      headers: {
  ...
  Future<Quiz> getQuizById(int quizId) async {
    final token = await TokenStorage.getToken();
    final response = await http.get(
      Uri.parse('https://375a-217-55-17-184.ngrok-free.app/api/v1/quizzes/$quizId'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json',
      },
