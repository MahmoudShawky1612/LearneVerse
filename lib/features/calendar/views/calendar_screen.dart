...
import 'package:flutterwidgets/features/community/service/single_community_service.dart';
...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  final SingleCommunityApiService _communityService =
      ...
      for (var classroom in reminder.community.classrooms) {
        for (var quiz in classroom.quizzes) {
          processedEvents.add({
            ...
            'communityId': reminder.community.id,
          });
        }
      }
    }
  ...
  Future<void> _navigateToCommunity(int communityId) async {
    try {
      final community =
          await _communityService.fetchSingleCommunity(communityId);
      if (mounted) {
        context.push('/community', extra: community);
      }
    } catch (e) {
      SnackBarUtils.showErrorSnackBar(context,
          message: "we have encoutered an error, can you try later ?");
    }
  }
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return Card(
                                    ...
                                    child: ListTile(
                                      ...
                                      subtitle: Text(
                                        '${_formatDate(event['day'])} - ${_formatDate(event['endDay'])}',
                                        style: GoogleFonts.poppins(
                                          fontSize: 12.w,
                                          color: colorScheme.onSurface
                                              .withOpacity(0.7),
