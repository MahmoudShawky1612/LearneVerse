...
import '../../service/user_contributions_service.dart';
...
class UserContributionsCubit extends Cubit<UserContributionsState> {
  ...
  UserContributionsCubit(this.userContributionsApiService)
      : super(UserContributionsInitial());

  void fetchContributionsByUser(int userId) async {
    emit(UserContributionsLoading());
    try {
      final contributions =
          await userContributionsApiService.fetchUserContributions(userId);
