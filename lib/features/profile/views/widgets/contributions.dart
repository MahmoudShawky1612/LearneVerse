...
class _ContributionChartState extends State<ContributionChart> {
  ...
  Widget build(BuildContext context) {
    ...
    final totalContributions =
        widget.contributions.fold<int>(0, (sum, c) => sum + c.count);
              ...
              if (day == 1 || day == 3 || day == 5) {
                return Padding(
                  padding: EdgeInsets.only(
                      bottom: day < 6 ? widget.cellSpacing.w : 0),
                  child: SizedBox(
                    width: 30.w,
          ...
          Expanded(
            child: ScrollConfiguration(
              ...
              child: SingleChildScrollView(
                ...
                child: SizedBox(
                  width: (widget.weeks *
                          (widget.cellSize.w + widget.cellSpacing.w)) -
                      widget.cellSpacing.w,
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      // Month labels
                      ...
                      // Grid
                      SizedBox(
                        height: (7 * widget.cellSize.w) +
                            (6 * widget.cellSpacing.w),
  ...
  List<String?> _getMonthLabels(int weeks) {
    ...
    if (dates.isEmpty) {
      ...
      for (int week = 0; week < weeks; week++) {
        ...
        if (week == 0 ||
            weekDate.month !=
                now.subtract(Duration(days: (week - 1) * 7)).month) {
    ...
    for (int week = 0; week < weeks; week++) {
      ...
      if (week == 0 ||
          weekDate.month !=
              startOfWeek.add(Duration(days: (week - 1) * 7)).month) {
        labels[week] = months[weekDate.month - 1];
      }
    }
    return labels;
  ...
  List<List<int>> _mapContributionsToGrid(
      List<UserContribution> contributions, int weeks) {
    ...
    for (final c in contributions) {
      final dateKey =
          "${c.dateOnly.year}-${c.dateOnly.month.toString().padLeft(2, '0')}-${c.dateOnly.day.toString().padLeft(2, '0')}";
