...
class ForumApiService {
  ...
  Future<ForumPost> createPost(int forumId, String title, String content,
  ...
  Future<String> uploadImage(File image) async {
    ...
    print(
        'Upload Image Response: ${response.statusCode} - $responseBody'); // Debug log
    if (response.statusCode == 201) {
  ...
  Future<List<Map<String, dynamic>>> fetchLeaderboardQuizScores(
      int communityId) async {
    final token = await TokenStorage.getToken();
    final url =
        Uri.parse('${ApiHelper.baseUrl}/leaderboard/quiz-score/$communityId');
    final response = await http.get(
      url,
      headers: {
