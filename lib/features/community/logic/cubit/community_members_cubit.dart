...
class CommunityMembersCubit extends Cubit<CommunityMembersStates> {
  ...
  CommunityMembersCubit(this.communityMembersApiService) : super(CommunityMembersInitial());

  Future<void> fetchCommunityMembers(int communityId) async {
    emit(CommunityMembersLoading());
    try {
      final members = await communityMembersApiService.fetchCommunityMembers(communityId);
